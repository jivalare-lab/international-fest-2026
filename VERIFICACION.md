...
**Brecha abierta (2 de 5).** Clasificado el 2026-09-11. Requerido 5, no 3: la pagina publica 31 correos y 26 telefonos personales de 70 personas, asi que un error toca a un tercero (regla 1). Decision explicita de Isaac tras plantearle la alternativa sin telefonos. Mitigacion: meta robots noindex + robots.txt, y la puerta comprueba que sigan ahi. Tiene git, CI EN VERDE (npm ci + las dos puertas, Node 22) y las puertas pasan 19 estaticas + 15 de render con jsdom. Nivel real 2, sube a 3 en cuanto Isaac confirme el CI. El formulario se sincroniza solo cada 3 horas, PERO la Action ABRE UN PULL REQUEST y no publica: asi la automatizacion no rompe la disciplina de nivel 2 de que Isaac apruebe el diff. Si un pais no se puede ubicar, sync.js aborta con codigo 1, la Action falla y llega correo. sync.js es idempotente: correrlo dos veces no produce diff. Para el 4 le falta vista previa del PR antes de mezclar. Para el 5 le faltan rastro de auditoria de que dato se publico y cuando, procedimiento probado de retirada a peticion de un host (incluye reescribir el historial de git) y alerta si la pagina se cae. PENDIENTE DE ISAAC: crear los secretos FORM_CSV_ESTUDIANTES y FORM_CSV_PROGRAMAS, sin ellos la Action falla a proposito. Clasificacion inicial del agente, pendiente de que Isaac la confirme.
